...
namespace TubieTools_Aspire.EnterpriseAutomation.Tests.MultiTenant
{
    ...
    {
        private readonly Mock<ILogger<TenantResolverMiddleware>> _mockLogger;
        ...
        public async Task InvokeAsync_WithJwtTenantIdClaim_ExtractsTenantId()
        {
            ...
            // Assert
            _mockTenantService.Verify(ts => ts.GetTenantAsync("jwt-tenant-001"), Times.GreaterThanOrEqualTo(0));
        }
        ...
        public async Task InvokeAsync_WithApiKeyInPath_ExtractsTenantId()
        {
            ...
            // Assert
            _mockTenantService.Verify(ts => ts.GetTenantAsync(It.IsAny<string>()), Times.GreaterThanOrEqualTo(0));
        }
        ...
        public async Task InvokeAsync_BuildsCorrectFeatureFlags_ForFreeTier()
        {
            ...
            _mockContextAccessor.Verify(
                ca => ca.SetTenantContext(It.Is<TenantContext>(tc =>
                    !tc.FeatureFlags["api_access"] &&
                    !tc.FeatureFlags["custom_prompts"] &&
                    !tc.FeatureFlags["multiple_agents"]
                )),
                Times.Once
            );
        }
